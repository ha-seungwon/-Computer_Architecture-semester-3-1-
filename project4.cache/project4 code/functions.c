...
int control(int opcode, int funct) {//function to initialize the control signal
    ...
    //printf("control regw,memtoreg,jal: %d %d %d\n", latch2[0].wb.RegWrite, latch2[0].wb.memtoReg, latch2[0].wb.j_and_l_toReg);
...
void memaccess() {
    ...
    //printf("latch4 : %d,%d\n", latch4[0].wb.memtoReg, latch4[0].wb.j_and_l_toReg);
}

int mem(int address, int writeData) {//memory
    if (latch3[1].m.memoryWrite == 1& latch3[1].m.memoryRead == 0) {//sw
        memory[address / 4] = writeData;
        switch (MappingType)
        {
        case 0 :
            FASWCache(address, writeData);
            break;
        case 1:
            DMSWCache(address, writeData);
            break;
        case 2:
            SWCache(address, writeData);
            break;
        case 4:
            SWCache4(address, writeData);
            break;
        case 8:
            SWCache8(address, writeData);
            break;
        }
    ...
    else if (latch3[1].m.memoryRead == 1 & latch3[1].m.memoryWrite == 0) {//lw
        int cacheresult = 0;
        switch (MappingType)
        {
        case 0:
            cacheresult = FALWCache(address);
            break;
        case 1:
            cacheresult = DMLWCache(address);
            break;
        case 2:
            cacheresult = LWCache(address);;
            break;
        case 4:
            cacheresult = LWCache4(address);
            break;
        case 8:
            cacheresult = LWCache8(address);
            break;
        }

        if (cacheresult != NULL) {       
            latch4[0].mReadData = cacheresult;           
        }
        else {
